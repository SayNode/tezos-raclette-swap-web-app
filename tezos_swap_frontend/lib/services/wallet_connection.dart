...
class WalletService extends GetxService {
  ...
  authorize(String tokenContract, String swapContract) async {
    bool isOp = await checkIfOperator(tokenContract, address.value, swapContract);
    if (!isOp) {
      String id = nanoid();
      html.window.addEventListener("message", (event) {
        final evt = (event as html.MessageEvent);
        if (evt.source == html.window && evt.data['reqId'] == id && evt.data['type'] == 'TEMPLE_PAGE_RESPONSE') {
          Get.close(1);
        }
      }, true);
            ...
            child: Padding(
              ...
              child: SizedBox(
                ...
                child: Card(
                  ...
                  shape: RoundedRectangleBorder(side: const BorderSide(color: Colors.white, width: 2.0), borderRadius: BorderRadius.circular(20.0)),
                  child: const Center(
...
//FIXME: might be problems with tokenx and y amount calculation
  swap(String contract, String recipient, double tokenX, double tokenY, String tokenXAddress, String tokenYAddress, {bool yToX = false}) async {
  ...
  setPosition(
    ...
    BigInt liquidity = etherToWei(await getLiquidity(yDouble, xDouble, lowerPrice, upperPrice, currentTick), 18);
  ...
  authorizeContract(String tokenContract, String swapContract, String id) async {
    _request({
      "type": "OPERATION_REQUEST",
  ...
  _request(Map payload, {String? id}) {
    ...
    html.window.addEventListener("message", (event) {
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Text(shortenHex(evt.data['payload']['opHash'])),
                                  GestureDetector(
                                    onTap: () => Clipboard.setData(ClipboardData(text: evt.data['payload']['opHash'])).then((_) {
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Hash has been copied to clipboard")));
                                    }),
      ...
      } else if (evt.source == html.window && evt.data['reqId'] == id && evt.data['type'] == 'TEMPLE_PAGE_ERROR_RESPONSE') {
        Navigator.of(Get.context!).pushReplacement(CardDialogRoute(
          builder: (context) {
            return Center(
              child: Padding(
...
//mehod to shorten hex strings
String shortenHex(String hex) {
  if (hex.length < 10) {
    return hex;
  }
